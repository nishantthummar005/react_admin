...
const Terms = () => {
    ...
    const [termsAllData, settermsAllData] = useState([]);
    const [getEditData, setEditData] = useState([{ _id: '', terms: '' }]);
    const [page, setPage] = useState(1);
    const [limit, setlimit] = useState(5);
    const context = useContext(termsContext);
    const { allTerms, getAllTerms, deleteTerms, addTermsRecord } = context;
    ...
    /* ************************************************
                    FETCH ALL RECORDS  // Fetch all terms & condition data through axios API
    ***********************************************/
    const getTermsList = () => {
        settermsAllData(allTerms);
    }

    /* ************************************************
                    INSERT RECORDS   // submit for inserting terms & condition data
    ***********************************************/
    const onSubmit = () => {
        addTermsRecord(getSummerval.trim(), page, limit); // form submit and insert record   
        setSummerval('')
    }
    ...
    const getEditItem = async (id) => {
        ...
        try {
            const response = await fetch(`${host}/api/terms/getedititem/${id}`, {
                method: 'GET',
                headers: {
    ...
    const setEditTextValue = (e) => {
        let newArr = { _id: getEditData._id, terms: e.target.value };   // assign value to the usestate  
        setEditData(newArr);
    }
                    ...
                    <div className="section-body">
                        {(pageType === "add") ? (
                            <div>
                                <div className="row">
                                    <div className="col-6">
                                        <h2 className="section-title mt-0">Add Terms</h2>
                                        <p className="section-lead">
                                            Change information Terms yourself on this page.
                                        </p>
                                    </div>
                                    <div className="col-6 text-right">
                                        <Link to="/clientPages/terms/show" className="btn btn-info">Show Terms</Link>
                                    </div>
                                </div>
                                            ...
                                            <div className="card-body">
                                                <form method="post" autoComplete="off">
                                                    <div className="form-group">
                                                        <label htmlFor="terms" className="form-label"><span className='text-danger'>*</span>Add Data</label>
                                                        <ReactSummernote
                                                            value={getSummerval}
                                                            options={{
                                                                lang: 'en',
                            ...
                            <ShowData getEditItem={getEditItem} page={page} setPage={setPage} limit={limit} setlimit={setlimit} termsAllData={termsAllData} deleteTerms={deleteTerms} />
                        )}
                    </div>
                </section>
            </div>
