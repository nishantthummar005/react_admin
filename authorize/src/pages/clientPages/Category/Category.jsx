...
const Category = () => {
    ...
    const [getEditData, setEditData] = useState([{ _id: '', category: '' }]);
    const [page, setPage] = useState(1);
    const [limit, setlimit] = useState(5);
    const context = useContext(categoryContext);
    const { allCategory, getAllCategory, deleteCategory, addCategoryRecord } = context;
    ...
    /* ************************************************
                    FETCH ALL RECORDS  // Fetch all category us data through axios API
    ***********************************************/
    const getCategoryList = () => {
        setcategoryAllData(allCategory);
    }
    ...
    ***********************************************/
    const onSubmit = data => {
        const { title } = data;
        addCategoryRecord(title.trim(), (getProfilePhoto) ? getProfilePhoto : '', page, limit); // form submit and insert record    
        reset({ title: '' });
        setProfilePhoto('');
    }
    ...
    const getEditItem = async (id) => {
        ...
        try {
            const response = await fetch(`${host}/api/category/getedititem/${id}`, {
                method: 'GET',
                headers: {
    ...
    const setEditTextValue = (e) => {
        let newArr = { _id: getEditData._id, category: e.target.value };   // assign value to the usestate  
        setEditData(newArr);
    }
                    ...
                    <div className="section-body">
                        {(pageType === "add") ? (
                            <div>
                                <div className="row">
                                    <div className="col-6">
                                        <h2 className="section-title mt-0">Add Category</h2>
                                        <p className="section-lead">
                                            Change information Category yourself on this page.
                                        </p>
                                    </div>
                                    <div className="col-6 text-right">
                                        <Link to="/clientPages/category/show" className="btn btn-info">Show Category</Link>
                                    </div>
                                </div>
                            ...
                            <ShowData getEditItem={getEditItem} page={page} setPage={setPage} limit={limit} setlimit={setlimit} categoryAllData={categoryAllData} deleteCategory={deleteCategory} />
                        )}
                    </div>
                </section>
            </div>
