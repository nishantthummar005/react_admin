...
const UpdateData = (props) => {
    ...
    const { updateBannerRecord } = context;
    ...
    useEffect(() => {
        setTimeout(() => setUser({ title: getEditData.title, subtitle: getEditData.subtitle }), 1000); // simulate async api call with set timeout 
    }, [getEditData]);
    ...
    ***********************************************/
    const onSubmit = data => {
        ...
        updateBannerRecord(getEditData._id, title.trim(), subtitle.trim(), (getProfilePhoto) ? getProfilePhoto : getEditData.path, page, limit); // form submit and insert record    
                ...
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                            ...
                            <form name='editForm' id="editForm" method="post" noValidate onSubmit={handleSubmit(onSubmit)} autoComplete="off" encType='multipart/form-data'>
                                ...
                                <div className="form-group col-md-12 col-12">
                                    ...
                                    <input
                                        type="text"
                                        id="subtitle"
                                        defaultValue={getEditData.subtitle}
                                        tabIndex={2}
                                        name="subtitle"
                                        className="form-control"
                                        {...register("subtitle", {
                        ...
                        <div className="modal-footer">
                            ...
                            <button type="submit" form='editForm' disabled={!getEditData.title && !getEditData.subtitle} className="btn btn-primary">Save changes</button>
                        </div>
                    </div>
                </div>
            </div>
